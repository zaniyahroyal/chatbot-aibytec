...
def chat_with_ai(user_question, website_text, pdf_text, chat_history):
    ...
    combined_context = f"""
    Website Overview:
    {website_text[:1000]}  # Trimming context to a manageable size
    PDF Summary:
    {pdf_text[:1000]}  # Trimming context to a manageable size
    """
    
    # Create conversation history
    messages = [{"role": "system", "content": "You are a concise and helpful assistant. Respond briefly and accurately."}]
    ...
    messages.append({"role": "user", "content": f"Context: {combined_context}\n\nQuestion: {user_question}"})
